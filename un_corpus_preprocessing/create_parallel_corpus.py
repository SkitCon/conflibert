...
Purpose: Script that uses the file UNv1.0.6way.ids to
    create a multilingual corpus between English,
    Spanish, and Arabic aligned at the sentence level.
    ...
    a new directory called UNv1.0-3way-linked
'''
...
def write_tasks_file_json(year, tasks):
    ...
    '''
    try:
        os.mkdir("UNv1.0-3way-linked")
    except OSError:
        pass
    
    with open(f"UNv1.0-3way-linked/{year}.json", 'w', encoding="utf-8") as f:
...
def write_tasks_file_csv(year, tasks):
    ...
    with open(f"UNv1.0-3way-linked/{year}.csv", 'w', encoding="utf-8") as csv_file:
        csv_writer = csv.writer(csv_file, quoting=csv.QUOTE_MINIMAL)
        csv_writer.writerow(["id","ar", "en", "es"])
        for task in tasks:
...
def main():
    ...
    elif len(sys.argv) == 2 or sys.argv[1] == "-json":
        write_tasks_file = write_tasks_file_json
    else:
        print("ERROR: Invalid command-line arguments. Use -csv or -json.")
        exit(1)
